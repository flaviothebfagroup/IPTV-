...
function applyTheme(theme) {
  ...
  if (type === "color") { body.style.background = color; return; }
  if (type === "gradient") {
    body.style.background = `radial-gradient(900px 520px at 25% -120px,rgba(201,150,60,0.14),transparent 60%),${color}`;
    return;
  }
  if (type === "image" && image) {
...
async function init() {
  ...
  // Apply avatarWidth/Height from profile settings (supports horizontal logos)
  const wrap = $("avatarWrap");
  if (wrap && data.profile) {
    const w   = Number(data.profile.avatarWidth  ?? data.profile.avatarSize ?? 72);
    const h   = Number(data.profile.avatarHeight ?? data.profile.avatarSize ?? 72);
    const pad = Number(data.profile.avatarPadding ?? 10);
    const rad = Number(data.profile.avatarRadius  ?? 18);
    wrap.style.width        = `${w}px`;
    wrap.style.height       = `${h}px`;
    wrap.style.padding      = `${pad}px`;
    wrap.style.borderRadius = `${rad}px`;
    wrap.style.boxSizing    = "border-box";
    if (data.profile.avatarScale) {
      avatar.style.transform       = `scale(${data.profile.avatarScale})`;
  ...
  (data.socials || [])
    ...
    .forEach(s => socialsWrap.appendChild(createSocial(s)));

  const linksWrap = $("links");
  linksWrap.innerHTML = "";
  (data.links || [])
    .filter(l => l && l.enabled !== false)
    .forEach(l => linksWrap.appendChild(createLink(l)));
